...
describe EventLog, type: :model do
  ...
  describe 'at_last_days' do
    before(:each) do
      10.times do |i|
        EventLog.create(severity: EventLog.severities['info'], subject: "subject-#{i}", body: "body-#{i}")
        EventLog.create(severity: EventLog.severities['warning'], subject: "subject-#{i}", body: "body-#{i}")
        EventLog.create(severity: EventLog.severities['error'], subject: "subject-#{i}", body: "body-#{i}")
      end
    ...
    it 'should return events created since given days ago' do
      ...
      expect(res.count).to be 11
      res = EventLog.at_last_days(25, 'info')
      expect(res.count).to be 25
      res = EventLog.at_last_days(25, 'warning')
